...
for Monkey_num=1:2 
    ...
    onset_gatherer_sg=cell(8,num_elec);
    onset_gatherer_fg=cell(8,num_elec);
    ...
    for i=current_electrode
        ...
        for ORI=1:8
            ...
            [length_temp_sg,~,time_center_sg,~,~,~]= getBurstLengthMP(data_temp,timeVals,thresholdFactor,displayFlag,stimulusPeriodS,baselinePeriodS,slow_gamma_freq,num_iterations,0.9,dict_size,gabor_temp,header_temp);
            ...
            onset_gatherer_sg{ORI,counter}=onset_temp_all_trials;
            ...
            thresholdFactor=thresholdFraction*sqrt(diffPower);
            [length_temp_fg,~,time_center_fg,~,~,~]= getBurstLengthMP(data_temp,timeVals,thresholdFactor,displayFlag,stimulusPeriodS,baselinePeriodS,fast_gamma_freq,num_iterations,0.9,dict_size,gabor_temp,header_temp);
            length_temp_all_trials=[];
            onset_temp_all_trials=[];
            for ii=1:length(length_temp_fg)
                ...
                time_center_fg{ii}(reject_idx)=[];
                length_temp_all_trials=[length_temp_all_trials,length_temp_fg{ii}'];
                % Find the first burst (in terms of time) in each trial
                % Onset time (earliest)
                onset_time_temp_trial_fg=time_center_fg{ii}'-((length_temp_fg{ii}')*0.5);
            ...
            onset_gatherer_fg{ORI,counter}=onset_temp_all_trials;
        end
    ...
    mean_onset_gatherer_sg=zeros(8,num_elec);
    mean_onset_gatherer_fg=zeros(8,num_elec);
    for ii=1:num_elec
        for ORI=1:8
            median_gatherer_sg(ORI,ii)=median(length_gatherer_sg{ORI,ii});
            median_gatherer_fg(ORI,ii)=median(length_gatherer_fg{ORI,ii});
            mean_onset_gatherer_sg(ORI,ii)=mean(onset_gatherer_sg{ORI,ii});
            mean_onset_gatherer_fg(ORI,ii)=mean(onset_gatherer_fg{ORI,ii});
            ...
            if (length(length_gatherer_sg{ORI,ii})<20) || (length(length_gatherer_fg{ORI,ii})<20)
                ...
                mean_onset_gatherer_sg(ORI,ii)=0;
                mean_onset_gatherer_fg(ORI,ii)=0;
                continue;
            end
    ...
    avg_onset_sg=zeros(8,1); avg_onset_fg=zeros(8,1);
    SEM_onset_sg=zeros(8,1); SEM_onset_fg=zeros(8,1);
    ...
    stats_val=cell(8,1);
    N_val=zeros(8,1);
    p_val_onset=zeros(8,1);
    h_val_onset=zeros(8,1);
    stats_val_onset=cell(8,1);
    N_val_onset=zeros(8,1);
    for ORI=1:8
        ...
        temp_onset_sg=mean_onset_gatherer_sg(ORI,:);
        temp_onset_sg=temp_onset_sg(temp_onset_sg~=0);
        temp_onset_fg=mean_onset_gatherer_fg(ORI,:);
        temp_onset_fg=temp_onset_fg(temp_onset_fg~=0);
        N_temp_onset=length(temp_onset_fg);
        N_val_onset(ORI)=N_temp_onset;
        [h_onset,p_onset,ci,stats_onset] = ttest(temp_onset_sg, temp_onset_fg, 'alpha',0.05,'tail','right');
        p_val_onset(ORI)=p_onset;
        h_val_onset(ORI)=h_onset;
        stats_val_onset{ORI}=stats_onset;
        ...
        avg_onset_sg(ORI)=mean(temp_onset_sg);
        avg_onset_fg(ORI)=mean(temp_onset_fg);
        SEM_onset_sg(ORI)=std(temp_onset_sg)/sqrt(length(temp_onset_sg));
        SEM_onset_fg(ORI)=std(temp_onset_fg)/sqrt(length(temp_onset_fg));
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    er=errorbar(ORI_axis,avg_onset_sg,SEM_onset_sg,'-o','MarkerSize',6,'LineWidth',1.5,'Capsize',10,'Color','b');
    hold on;
    errorbar(ORI_axis,avg_onset_fg,SEM_onset_fg,'-o','MarkerSize',6,'LineWidth',1.5,'Capsize',10,'Color',color_orange);
    signific_val_onset=p_val_onset<0.05;
    %%% For significant p-values, the markerface color of that corresponding ORI is changed
    for i=1:8
        if signific_val_onset(i)==1
            plot(ORI_axis(i),avg_onset_sg(i),'o','MarkerSize',6,'MarkerFaceColor','r','MarkerEdgeColor','b','LineWidth',1.5,'Color','b');
            plot(ORI_axis(i),avg_onset_fg(i),'o','MarkerSize',6,'MarkerFaceColor','g','MarkerEdgeColor',color_orange,'LineWidth',1.5,'Color',color_orange);
        end
    end
    ...
    if Monkey_num==1
       save("stats_new_sup_fig_4_M1.mat","p_val","h_val","stats_val","N_val",...
           "N_val_onset","p_val_onset","h_val_onset","stats_val_onset");
    elseif Monkey_num==2
       save("stats_new_sup_fig__M2.mat","p_val","h_val","stats_val","N_val",...
           "N_val_onset","p_val_onset","h_val_onset","stats_val_onset");         
    end
