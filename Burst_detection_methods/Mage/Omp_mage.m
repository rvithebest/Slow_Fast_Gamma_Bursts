function [b,a,YFIT] = Omp_mage(signal,dictSize,sRate,iterMax)
    % make dictionary %
    t_start=tic;
    L=length(signal);
    ...
    for k=1:iterMax
        ...
        mx = scalProdcos(i);
        phase = atan2(my,mx);
        [gparams] = mage_reassign_gparams_td(R,sRate,[t(kopt),f(kopt),2*log(s(kopt))]);
        tnw=gparams(1);
        fnw=gparams(2);
        snw=exp(gparams(3)/2);
...
%             %plot(R,'b')
%             plot(scalProdcos(i)*mageAtom_real([t(kopt),f(kopt),2*log(s(kopt))],sRate,L,0),'k');
