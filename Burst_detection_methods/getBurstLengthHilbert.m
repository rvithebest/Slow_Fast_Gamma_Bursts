function [burstLengthS, timeCenter] = getBurstLengthHilbert(analogData,timeVals,thresholdFactor,displayFlag,stimulusPeriodS,baselinePeriodS,burstFreqRangeHz,filterOrder)
    ...
    for i=1:numTrials
        ...
        [burstLengthS{i},timeCenter{i}] = getBurstLengthHilbertSingleTrial(hilbertPower(i,:),timeVals,threshold,stimulusPeriodS,displayFlag);
    end
    
    function [burstLengthS,timeCenter] = getBurstLengthHilbertSingleTrial(bandPassPowerSingleTrial,timeVals,threshold,stimulusPeriodS,displayFlag)
    ...
    timeCenter = (timeVals(burstStartPosList) + timeVals(burstEndPosList)) / 2;
    if displayFlag==1
