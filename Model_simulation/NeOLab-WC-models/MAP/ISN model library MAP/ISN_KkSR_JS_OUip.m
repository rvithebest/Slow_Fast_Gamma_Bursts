...
function pop = ISN_KkSR_JS_OUip(nsimulations, inputfile, taus, theta,Weights, A)
    ...
    if ~exist('taus','var')
        % taus = (10/13)*([20,10]*1e-3); % seconds- fast gamma- OU noise 2
        % taus = (20/13)*([20,10]*1e-3); % seconds- slow gamma- OU noise 2
        taus = (10/13)*([20,20]*1e-3); % seconds- slow gamma- OU noise 3
        % taus=[16,8]; % Fast- OU noise new- did not work
    end
