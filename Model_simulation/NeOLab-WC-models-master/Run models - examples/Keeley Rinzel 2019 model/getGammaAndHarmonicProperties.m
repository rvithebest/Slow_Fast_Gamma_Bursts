...
function [peakGammaFreq,harmonicFreq,freqRatio,gammaAmp,harmonicAmp,powerRatio,hilbPhaseDiff, freqVals, fftx]=getGammaAndHarmonicProperties_JS_RKtunedFeb2022(x,gammaRangeHz,gammaBandwidthHz,tMS,xE,xI)
...
%     gammaPhase = angle(fftx(gammaPos));
    ...
    for i =1:size(x,1)
        ...
        [m,i] = max(abs(fftHarmonic), [], 2);
        harmonicAmp = [harmonicAmp;m];
        harmonicPos = [harmonicPos;harmonicRangePos(i)];
    end
