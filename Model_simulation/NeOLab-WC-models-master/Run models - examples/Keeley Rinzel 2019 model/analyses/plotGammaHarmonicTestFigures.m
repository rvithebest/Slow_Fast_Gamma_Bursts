...
if ~dontPlot
    ...
    goodHarmonicPos = find(harmonicAmp > 1e-2);
    goodGammaPos = find(peakAmp > 1e-2);
    goodPos = intersect(goodHarmonicPos,goodGammaPos);
    ...
    zeroPhaseDiffregion = (reshape(hilbPhaseDiffSelected,[lI, lE]) <= 0+ interval) | (reshape(hilbPhaseDiffSelected,[lI, lE]) >= 360-interval);
    ...
    subplot(236)
    colormap(gca, hsv);
    pcolorSurface = pcolor(e0,i0,(reshape(hilbPhaseDiffSelected,[lI, lE]))); % pcolorSurface.FaceColor = 'interp';
    set(gca, 'Color', [0.5, 0.5, 0.5]);
    % xlabel('iE','FontWeight','bold'); ylabel('iI','FontWeight','bold');
    colormap(gca, hsv);
    colorbar;
    ...
    hsvdata(noNanIndices,:) = hsvmap(round(hilbPhaseDiffSelected(noNanIndices)/360*(size(hsvmap,1))+1),:);
    ...
    Phasediff_regime = hilbPhaseDiffSelected(ingoodpos & (zeroPhaseDiffregion));
